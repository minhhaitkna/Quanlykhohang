...
public class RevenueGraphController {
    ...
    private CategoryDataset createDataset(Timestamp start, Timestamp end) throws SQLException {
        ...
        ArrayList<Statistical.EmployeeIncome> incomes = statisticsDao.getListTotalIncomeByDate(start, end);
        for (Date date : getAllDateBetween(start, end)) {
            Statistical.EmployeeIncome income = findByDate(incomes, date);
            if (income != null && income.date != null) {
                dataset.addValue(income.totalIncome / 100000.0, "TN", formatter.format(income.date));
            } else {
    ...
    private static ArrayList<Date> getAllDateBetween(Timestamp start, Timestamp end) {
        ...
        while (!i.after(endTime)) {
            dates.add(i);
            i = new Date(i.getTime() + 24L * 60 * 60 * 1000);
        }
        return dates;
    }
    ...
    public void show(JPanel panelRoot, Timestamp start, Timestamp end) throws SQLException {
        ...
        JPanel chartPanel = GraphLine.createChartPanel(dataset);
        panelRoot.removeAll();
